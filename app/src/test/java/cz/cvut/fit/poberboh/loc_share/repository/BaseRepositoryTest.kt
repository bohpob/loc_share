...
class BaseRepositoryTest {
    ...
    fun safeApiCall_shouldReturnResourceSuccess_whenApiCallSucceeds() = runTest {
        ...
        Assert.assertEquals(expectedData, (result as Resource.Success).data)
    }
    ...
    fun safeApiCall_shouldReturnResourceError_forConnectException() = runTest {
        ...
        Assert.assertEquals(false, (result as Resource.Error).isNetworkError)
        Assert.assertEquals("Failed to connect", result.errorBody?.string())
    }
    ...
    fun safeApiCall_shouldReturnResourceError_forOtherExceptions() = runTest {
        ...
        // Assert
        Assert.assertTrue(result is Resource.Error)
        Assert.assertEquals(true, (result as Resource.Error).isNetworkError)
    }
}